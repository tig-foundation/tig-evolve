...
// TIG's UI uses the pattern `tig_challenges::<challenge_name>` to automatically detect your algorithm's challenge
        ...
        Some(params) => {
            serde_json::from_value::<Hyperparameters>(Value::Object(params.clone()))
                .unwrap_or(Hyperparameters {
                    max_flips: 100000000, // Significantly increased restarts for deeper search
                    early_limit: 4000000, // Increased stagnation limit
                    noise: 0.52, // Increased factor for faster escape from deep local optima
                    early_factor: 0.9, // RCL threshold (0.98 means highly greedy)
                })
        }
    ...
    solve_with_walksat_vav(challenge, &mut rng, save_solution, _hyperparameters)
...
fn solve_with_walksat_vav(
    ...
    // --- Stage 1: Genetic Algorithm for initial global search ---
    let (ga_best, _, ga_solved) = run_ga(
        &instance,
        &fixed,
        &pos_counts,
        &neg_counts,
    ...
    if ga_solved {
        let variables = finalize_assignment(challenge.num_variables, &fixed, Some(&ga_best), rng);
        save_solution(&Solution { variables })?;
        return Ok(());
    }

    // --- Stage 2: Ant Colony Optimization to refine GA's best solution ---
    let (aco_best, _, aco_solved) = run_aco(
        &instance,
        &fixed,
        &pos_counts,
        &neg_counts,
    ...
    if aco_solved {
        let variables = finalize_assignment(challenge.num_variables, &fixed, Some(&aco_best), rng);
        save_solution(&Solution { variables })?;
        return Ok(());
    }

    // --- Stage 3: WalkSAT for intense local search from ACO's best solution ---
    ...
    let (best_assignment, _, _) = walksat_search(
        &instance,
        &fixed,
        &pos_occ,
        &neg_occ,
        aco_best, // Use ACO's best as the starting point
        rng,
        max_flips,
        noise,
        early_factor,
