...
pub fn solve_challenge(
    ...
    pub struct Hyperparameters {
        ...
        pub rcl_alpha: f64, // Parameter for Restricted Candidate List in repair/construction
    }
        ...
        Some(params) => {
            serde_json::from_value::<Hyperparameters>(Value::Object(params.clone()))
                .unwrap_or(Hyperparameters {
                    ...
                    rcl_alpha: 0.05, // 5% tolerance for density
                })
        }
    ...
    // --- 1. Initial Greedy Construction Phase (O(N^2)) using RCL ---
    loop {
        let mut best_density: f64 = f64::NEG_INFINITY;
        let mut candidate_densities = Vec::new(); // (density, index)
            ...
            if !is_selected[i] {
                ...
                if current_weight.checked_add(item_weight).map_or(false, |w| w <= max_weight) {
                    ...
                    if density > f64::NEG_INFINITY {
                        candidate_densities.push((density, i));
        ...
        if best_density <= 0.0 {
            // If the best possible gain is zero or negative, stop construction.
            break;
        }
        
        // Build Restricted Candidate List (RCL)
        let threshold = best_density * (1.0 - hyperparameters.rcl_alpha);
        let rcl: Vec<usize> = candidate_densities.into_iter()
            .filter(|&(density, _)| density >= threshold)
            .map(|(_, index)| index)
            .collect();
        ...
        let idx_to_add = rcl[rng.gen_range(0..rcl.len())];
    ...
    for restart in 0..hyperparameters.max_restarts {
        ...
        for _ in 0..p_size {
            ...
            // Calculate MG_max and Total Weight of selected items for normalization (O(N_selected))
            let mut mg_max = i64::MIN;
            let mut total_w_selected: u64 = 0;
            let n_selected_current = selected_indices.len();

            for &i in selected_indices.iter() {
                mg_max = mg_max.max(marginal_gains[i]);
                total_w_selected += weights[i] as u64;
            }
            
            mg_max = mg_max.max(1);
            let avg_w_selected = if n_selected_current > 0 { 
                (total_w_selected / n_selected_current as u64).max(1) as i64 
            } else { 1 };

            // Calculate weights: S_j = (MG_max - MG_j + 1) * (W_j / W_avg) (Bias towards low MG and heavy items)
            let mut total_weight = 0i64;
            let weights_and_indices: Vec<(i64, usize)> = selected_indices.iter().map(|&j| {
                // MG contribution (S_mg >= 1)
                let s_mg = mg_max - marginal_gains[j] + 1; 
                
                // Weight contribution factor (W_j / W_avg, minimum 1)
                let w_j = weights[j] as i64;
                let weight_factor = w_j.checked_div(avg_w_selected).unwrap_or(1).max(1);
                
                // Combined weight: prioritize low MG, amplified by relative heaviness
                // Use checked multiplication for safety, falling back to a large cap if overflow occurs.
                let weight = s_mg.checked_mul(weight_factor).unwrap_or(i64::MAX / 2);
        ...
        // 3. Greedy Repair (Add items back based on marginal density using RCL)
        loop {
            let mut best_density: f64 = f64::NEG_INFINITY;
            let mut candidate_densities = Vec::new(); // (density, index)
