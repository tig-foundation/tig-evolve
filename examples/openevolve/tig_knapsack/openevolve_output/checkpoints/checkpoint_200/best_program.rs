...
pub fn solve_challenge(
    ...
    pub struct Hyperparameters {
        ...
        pub repair_alpha: f64, // Parameter for Restricted Candidate List (RCL) repair
    }
        ...
        Some(params) => {
            serde_json::from_value::<Hyperparameters>(Value::Object(params.clone()))
                .unwrap_or(Hyperparameters {
                    max_restarts: 1000, // Significantly increased restarts for deeper search
                    base_perturbation_size: 15, // Slightly smaller base perturbation
                    stagnation_limit: 25, // Increased stagnation limit
                    perturbation_increase_factor: 1.5, // Increased factor for faster escape from deep local optima
                    repair_alpha: 0.98, // RCL threshold (0.98 means highly greedy)
                })
        }
        None => Hyperparameters {
            max_restarts: 1000,
            base_perturbation_size: 15,
            stagnation_limit: 25,
            perturbation_increase_factor: 1.5,
            repair_alpha: 0.98,
        },
    };
                    ...
                    for i in 0..num_items {
                        if !is_selected[i] {
                            ...
                            if (*current_weight as i64 + delta_w) <= max_weight as i64 {
                                ...
                                // Delta V(j out, i in) = MG[i] - interactions[i][j] as i64 - MG[j]
    ...
    loop {
        ...
        for i in 0..num_items {
            if !is_selected[i] {
                ...
                if current_weight.checked_add(item_weight).map_or(false, |w| w <= max_weight) {
                    ...
                    if density > 0.0 { // Only consider items with positive marginal gain
                        candidates.push((density, i));
                    }
                }
            }
        }
        ...
        if best_density <= 0.0 {
            break; // No positive gain item can be added
        }

        // Build Restricted Candidate List (RCL)
        let threshold = best_density * hyperparameters.repair_alpha;
        let rcl: Vec<usize> = candidates.into_iter()
            .filter(|&(density, _)| density >= threshold)
            .map(|(_, i)| i)
            .collect();

        if !rcl.is_empty() {
            let chosen_index = rng.gen_range(0..rcl.len());
            let idx_to_add = rcl[chosen_index];
