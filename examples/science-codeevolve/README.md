...
After running CodeEvolve, the best program is saved in the `experiments/config/<island_id>/` folder. To test this:

```bash
# Copy the best program to algo-runner
cp examples/science-codeevolve/tig_<challenge>/experiments/config/<island_id>/best_sol.rs algo-runner/src/algorithm/mod.rs

python3 tig.py build_algorithm
python3 tig.py test_algorithm <track> --tests 1000
```

Replace `<challenge>` with your challenge name, `<island_id>` with the island ID with the best solution (e.g., `0`, `1`), and `<track>` with the appropriate track for your challenge (e.g., `"n_items=500,density=25"` for knapsack).
...
evaluate('../experiments/config/<island_id>/best_sol.rs', 'results.json')
"
cat results.json
```
